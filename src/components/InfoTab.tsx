...
export function InfoTab({ serverName, serverConfig, serverState, width, height, focused = false }: InfoTabProps) {
  ...
  return (
    <Box width={width} height={height} flexDirection="column" paddingX={1}>
      <Box paddingY={1} flexShrink={0}>
        <Text bold backgroundColor={focused ? 'yellow' : undefined}>Info</Text>
      </Box>
      ...
      {serverName ? (
        <>
          ...
          <Box height={height - 4} overflow="hidden" paddingTop={1}>
            ...
            {serverConfig ? (
              <Box flexShrink={0} marginTop={1} paddingLeft={2} flexDirection="column">
                {serverConfig.type === undefined || serverConfig.type === 'stdio' ? (
                  <>
                    <Text dimColor>Type: stdio</Text>
                    <Text dimColor>Command: {(serverConfig as any).command}</Text>
                    {(serverConfig as any).args && (serverConfig as any).args.length > 0 && (
                      <Box marginTop={1} flexDirection="column">
                        <Text dimColor>Args:</Text>
                        {(serverConfig as any).args.map((arg: string, idx: number) => (
                          <Box key={`arg-${idx}`} paddingLeft={2} marginTop={idx === 0 ? 0 : 0}>
                            <Text dimColor>{arg}</Text>
                          </Box>
                        ))}
                      </Box>
                    )}
                    {(serverConfig as any).env && Object.keys((serverConfig as any).env).length > 0 && (
                      <Box marginTop={1}>
                        <Text dimColor>
                          Env: {Object.entries((serverConfig as any).env).map(([k, v]) => `${k}=${v}`).join(', ')}
                        </Text>
                      </Box>
                    )}
                  ...
                  <>
                    ...
                    {(serverConfig as any).headers && Object.keys((serverConfig as any).headers).length > 0 && (
                      <Box marginTop={1}>
                        <Text dimColor>
                          Headers: {Object.entries((serverConfig as any).headers).map(([k, v]) => `${k}=${v}`).join(', ')}
                        </Text>
                      </Box>
                    )}
                  </>
            ...
            {/* Server Info */}
            {serverState && serverState.status === 'connected' && serverState.serverInfo && (
              <>
                ...
                <Box flexShrink={0} marginTop={1} paddingLeft={2} flexDirection="column">
                  {serverState.serverInfo.name && (
                    <Text dimColor>Name: {serverState.serverInfo.name}</Text>
                  )}
                  {serverState.serverInfo.version && (
                    <Box marginTop={1}>
                      <Text dimColor>Version: {serverState.serverInfo.version}</Text>
                    </Box>
                  )}
