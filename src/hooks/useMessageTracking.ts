...
export function useMessageTracking() {
  ...
  const pendingRequestsRef = useRef<Map<string | number, { timestamp: Date; serverName: string }>>(new Map());

  const trackRequest = useCallback((serverName: string, message: JSONRPCRequest) => {
  ...
  const trackResponse = useCallback((serverName: string, message: JSONRPCResultResponse | JSONRPCErrorResponse) => {
    ...
    if (pending && pending.serverName === serverName) {
      ...
      setHistory((prev) => {
        ...
        const requestIndex = serverHistory.findIndex(
          (e) => e.direction === 'request' && 'id' in e.message && e.message.id === entryId
        );
  ...
  const trackNotification = useCallback((serverName: string, message: JSONRPCNotification) => {
    const entry: MessageEntry = {
      id: `${serverName}-${Date.now()}-${Math.random()}`,
      timestamp: new Date(),
