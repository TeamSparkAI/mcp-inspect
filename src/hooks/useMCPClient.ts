...
class LoggingProxyTransport implements Transport {
  ...
  set onmessage(handler: (<T extends JSONRPCMessage>(message: T, extra?: MessageExtraInfo) => void) | undefined) {
    if (handler) {
      ...
      this.baseTransport.onmessage = <T extends JSONRPCMessage>(message: T, extra?: MessageExtraInfo) => {
        // Track incoming messages
        if ('id' in message && message.id !== null && message.id !== undefined) {
          // Check if it's a response (has 'result' or 'error' property)
          if ('result' in message || 'error' in message) {
            this.callbacks.trackResponse?.(message as JSONRPCResultResponse | JSONRPCErrorResponse);
